...
whether rewriting an AI-sounding draft (`fix`) or drafting new copy that reads
human from the start (`generate`). It works for any genre (reports, docs,
marketing, blog posts, emails, academic prose, fiction) by matching that genre's
conventions (Register profiles): a universal core of tells is fixed everywhere;
the rest flex.
...
obviously AI. Fix the deep tells first. The skill draws on the same ideas as the
public detectors (GPTZero, proselint, write-good, Vale) but goes past them on
substance, stance, and consistency, which no word list catches. Banned-word lists
age, so treat lexical checks as a floor, never proof.

## What detectors actually measure (and how we beat them honestly)

AI-text detectors combine measurable signals: **perplexity** (AI picks
high-probability tokens, so text reads "too predictable"), **burstiness**
(sentence-length variance, the single biggest mechanical lever, reported as the
linter's coefficient of variation), **n-gram repetition / lexical diversity**,
and **stylometry** (uniform openers, "not X, but Y", rule-of-three,
tailing-significance clauses). We optimize each by writing *genuinely better*,
never by gaming. The full landscape, the newer detectors (Binoculars, DetectGPT,
Ghostbuster), and sources are in [`references/ai-tells.md`](references/ai-tells.md).

**Scope and ethics.** The promise is *reads as written by a skilled human*, not
*disguise machine text*. Never use adversarial evasion tricks: Unicode
homoglyphs, zero-width characters, deliberate typos, meaning-degrading synonym
swaps, or fabricated facts/quotes/stats. Those wreck the text and violate
principle 3. Detectors are also demonstrably unreliable in ways that matter
ethically: Liang et al. (2023) found they disproportionately misclassify
non-native-English writing as AI. That is the strongest reason no detector is
ground truth, and another reason the goal is genuinely better writing, not a
passing score.
...
8. **Be honest about measurement.** The linter is a floor (regex-able surface
   features, not computed perplexity or curvature); your judgment is the ceiling
   (vacuity, weak stance, fabrication no regex can see). No detector is ground
   truth. Report a score, but state plainly that the real test is a skeptical
   human read.
...
If a file path is given, do not overwrite it blindly: confirm it is git-tracked
(recoverable) or write a `<file>.bak` first, show the rewrite/diff plus the audit,
then edit in place once the rewrite passes the loop. For pasted text (no file),
print the rewrite plus the audit; write no file. Register is inferred from the
content (Register profiles); ask one short question only when genuinely ambiguous.

## Register profiles

"Human" depends on genre. Infer the register, then apply the universal core
(principle 5) plus that register's conventions. Pass the matching `--register` to
the linter so it mutes checks that don't apply (e.g. warmth in marketing). The
universal core is fixed in every profile; only warmth, address, contractions,
hedging, and structural strictness flex by register.
...
Across every register, principles 3 and 4 still bind: never fabricate, and match
the genre's voice rather than bolting on one it rejects.

## Top tells (curated)

The highest-signal tells, with one-line fixes. The **full quick index** (every
tell) and the catalog with BAD → GOOD pairs per category live in
...
- **Dashes** → the loudest tell (full rule in principle 2). Outside `creative`,
  replace nearly all em-dashes with a varied mark; keep the hyphen for compounds
  and the en-dash for ranges (10–20); never `--` or a spaced ` - `. See category 9.
...
- **Meta-commentary / chatbot scaffolding** ("This report aims to…", "Great
  question! Let's dive in") → delete; open on the content.
- **Empty conclusions** ("In conclusion, X is a powerful tool…") → end on the
  last real point.
- **Uniform sentence length / low burstiness** → add short punches against the
  long sentences.
- **False agency** (abstract subject + human verb: "the complaint becomes a fix",
  "the data tells us") → name the human who acted, or use "you"; never invent an
  actor. Muted for `academic` ("the data show").
- **Fabricated specificity** ("up to 40%") with no source → cite or cut; never
  invent.
- **Puffery / hype** ("stands as a testament", "world-class") → a concrete claim,
  or cut.
...
- **Filler** (delve, leverage, robust, seamless, crucial, comprehensive,
  landscape) → the plain word, or cut.
- **Terminology / dialect / voice drift** → one term per concept, one dialect,
  one heading convention, one author voice throughout.

See the reference quick index for the rest (over-signposting, hedging stacks,
agent self-narration, narrator-from-a-distance, negative listing, performative
fragmentation, Wh-opener crutch, tailing-significance clause, redundancy, doubled
words, emoji/decorative bold, punctuation mechanics).

Worked BAD → GOOD anchors for the highest-signal tells (vacuity, rule-of-three +
puffery, buried verdict, chatbot scaffolding, false agency) live with a pair for
every category in [`references/ai-tells.md`](references/ai-tells.md) — load it for
the rewrite.
...
2. **Five edit moves, in order** (principle 1; expanded in the Rewrite
   procedure): ① cut vacuity → ② vary rhythm → ③ dismantle templates → ④ cut
   stance tells (and sharpen the verdict) → ⑤ fix diction and jargon last.
...
Throughout, principles 3 and 4 bind: never fabricate, and match the register.
...
3. **Cut vacuity.** Delete sentences and paragraphs that carry no information.
   On padded drafts this often removes 15–25% of the words and most of the "AI
   feel" at once, but cut only genuinely empty text, not to hit a ratio.
...
7. **Sharpen the evaluation** (principle 7; the deepest human-vs-AI signal). Make
   the text take a position: commit to a recommendation, weight lopsided tradeoffs,
   lead with the verdict, give the mechanism, call a wrong choice wrong. Naming
   genuine limits ("not tested on multi-node") is honest stance, not hedging.
   Category 8 in `references/ai-tells.md`.
8. **Unify voice and materials** (principle 6). One author voice, one term per
   concept (no "the model" → "the LLM" → "the network"), one dialect, one
   heading-case, one tense for findings, consistent number/list formatting.
   Category 7.
9. **Fix diction, jargon, and mechanics.** Replace filler, clichés, and business
   jargon with the plain word the meaning needs, or cut (Anti-jargon rules). Fix
   dashes and punctuation per principle 2 and Top tells: no doubled words, no
   space before `,;:!?`, one terminal mark. Running the linter with `--fix` clears
   the mechanical ones first. Skip swaps that leave the sentence vague. Category 9.
10. **Calibrate to the register** (principle 4, Register profiles). Match the
    genre's conventions and hold them end to end; add what the genre wants
    (contractions in casual, "you" in marketing); never bolt on a voice it rejects.
...
**Reference targets** (typical of good output, not quotas to force; an
already-tight input may legitimately miss them):
- burstiness CoV ≥ ~0.5 (sentence-length variation);
- em-dash density ≈0 outside `creative` (replace nearly all with varied marks);
- in `fix` mode, cutting 15–25% of the words is common, but cut only what is
  vacuous or redundant — never pad-then-trim a lean draft to hit a number;
...
1. **Run the hallucination pass.** Diff the rewrite's claim inventory against the
   source's (Anti-hallucination protocol, step 8). Any new, strengthened,
...
5. **Stop at a fixed point, capped at 3 passes.** Stop when a pass produces no net
   improvement or no dimension is below the bar, whichever comes first. The cap
   bounds cost; it is not a target. If tells remain after 3, report them honestly
   in the audit rather than over-correcting.
...
"fixed" with an invented detail, a hedge becomes a confident false claim, a vague
gesture grows a fake statistic. The rewrite must add *voice*, never *facts*.
...
A regex cannot catch a fabricated fact, so this protocol is judgment, not a linter
check: the linter only flags the vague-attribution and fabricated-specificity
phrasings that often accompany it.

## Anti-jargon rules

Jargon is the corporate cousin of filler (the linter flags a `jargon` category:
"synergy", "leverage", "circle back", "move the needle", "low-hanging fruit",
"best-in-class", "operationalize"). Cutting words is not enough; apply the rules.
...
2. **Keep necessary technical terms; cut empty business jargon.** A precise
   ...
   the word *does work*.
3. **Define a necessary term once, then reuse it** in plain language (this also
   satisfies one-term-per-concept, principle 6). Don't rotate synonyms.
4. **No buzzword stacking.** One borderline term is a judgment call; three
   ("leverage our synergies to operationalize best-in-class solutions") is always
   wrong. Rewrite the sentence around what it actually claims.
5. **Watch the register.** Marketing tolerates light enthusiasm but *not* empty
   jargon (its specific AI failure mode); a technical report tolerates dense
   terminology but not business-speak. Never use jargon for a concrete claim.

## Anti-overcorrection guardrails

- **Do** keep the occasional tricolon and transition where natural (the em-dash
  is the one exception, per principle 2).
...
2. **Baseline lint** (skip for `generate`'s first draft):
   ...
   and quote both numbers in the audit (`--baseline` prints the delta). The script
   covers surface tells and consistency drift (phrase/spelling lists in
   `scripts/ai_prose_patterns.json`); it is the deterministic floor only and cannot
   see vacuity, weak stance, or terminology drift. If the script is unavailable,
   degrade to pure judgment using `references/ai-tells.md` and this **no-tool
   checklist**: (a) read the
...
4. **Rewrite** per the procedure above, loading `references/ai-tells.md`, then run
   the **self-critique loop** until clean or 3 passes.
5. **Apply / present** (file handling per Modes), always printing the Humanization
   Audit, and **confirm invariants** with a diff (`git diff` or a before/after of
   numbers, code, links).
